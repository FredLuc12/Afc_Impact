...
def dashboard_page(installation_id: str | UUID | None = None) -> None:
    ...
    def content() -> None:
        ...
        alertes_actives = [a for a in alertes if a.get('statut') == 'active']
        ...
        with ui.row().classes('gap-3 wrap w-full'):
            ...
            with ui.card().classes('p-4 min-w-[180px]'):
                ui.label('Alertes actives').classes('text-xs text-gray-500')
                ui.label(str(len(alertes_actives))).classes('text-2xl font-bold')

            with ui.card().classes('p-4 min-w-[180px]'):
                ui.label('Dernier mode').classes('text-xs text-gray-500')
                ui.label(str(latest_choix.get('mode', '—'))).classes('text-xl font-bold')
        ...
        with ui.card().classes('w-full p-4'):
            ...
            if latest_choix:
                ui.label(f"Énergie choisie : {latest_choix.get('energie_choisie', '—')}")
                ui.label(f"Score PAC : {latest_choix.get('score_pac', '—')}")
                ui.label(f"Score Gaz : {latest_choix.get('score_gaz', '—')}")
                ui.label(f"Raison : {latest_choix.get('raison', '—')}")
            else:
