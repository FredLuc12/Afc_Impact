...
def alertes_page(installation_id: str | UUID | None = None) -> None:
    ...
    def content() -> None:
        ...
        # Le service doit maintenant retourner les alertes liées aux capteurs de cette installation
        ...
        if not alertes:
            ...
            with ui.element("div").classes("w-full flex flex-col gap-3"):
                for alerte in alertes[:10]:
                    # Extraction de la source (Nom du capteur récupéré via la jointure)
                    # On suppose que le service renvoie 'capteurs' -> 'nom'
                    capteur_info = alerte.get("capteurs", {})
                    source_name = capteur_info.get("nom") if isinstance(capteur_info, dict) else alerte.get("source", "Capteur inconnu")

                    with ui.card().classes("w-full border-l-4 border-coral p-3 shadow-sm"):
                        with ui.row().classes("w-full justify-between items-center"):
                            ui.label(alerte.get("titre", "Anomalie détectée")).classes("font-bold text-slate-700")
                            
                            level = (alerte.get("niveau") or "info").lower()
                            badge_type = "danger" if level == "critical" else "warning" if level == "warning" else "info"
                            render_alert_badge(level.upper(), badge_type)

                        ui.label(alerte.get("message", "Une anomalie a été relevée sur le capteur.")).classes("text-sm text-slate-500 my-1")
        ...
        # Statistiques
        ui.label("Résumé des événements").classes("text-lg font-bold mt-6 mb-2")
        render_stat_card(
            "Alertes par sévérité",
            [
                ("Critiques", f"{len([a for a in alertes if (a.get('niveau') or '').lower() == 'critical']):02d}"),
                ("Warnings", f"{len([a for a in alertes if (a.get('niveau') or '').lower() == 'warning']):02d}"),
                ("Infos", f"{len([a for a in alertes if (a.get('niveau') or '').lower() == 'info']):02d}"),
            ],
        )
